//mqtt --> artik cloud example
//send information is Cds value in acd0
...
int main(int argc, FAR char *argv[]) {
	...
	while(1){
		...
		if(publishedVal - value > 50 || value - publishedVal > 50){
			printf("send publish message, value is %d\n", value);
			char buf[40];
			sprintf(buf, "{\"brightness\" : %d}", value);
			mqtt_msg_t msg;
			msg.payload = (char*)buf;
			msg.payload_len = strlen(buf);
			msg.topic = strTopicMsg;
			msg.qos = 0;
			msg.retain = 0;

			int success = mqtt_publish(pClientHandle, msg.topic, (char*)msg.payload, msg.payload_len, msg.qos, msg.retain);
			if(success < 0){
				printf("publish fail\n");
			}
			else{
